...
### 2.0. 보안 (신규)
- **로그인 시스템:** `st.secrets` 기반 비밀번호 인증 (`admin1!`). 인증 성공 시에만 앱 데이터 로드.

### 2.1. 여행 준비 탭
- **모바일 초밀착 최적화:** 폰 접속 시 상단 여백 제거(0rem), 스트림릿 헤더 숨김, 제목/버튼 한 줄 배치 등 폰 화면에 최적화된 레이아웃.
- **가독성 위계:** 폰 한정 폰트 크기 재설정(h1: 25px, h2: 19px, 본문/버튼: 14.5px).
...
- `상세일정`: 날짜, 요일, 시간, 확정(ok/빈값), 내용, 메모, 장소명, 구글지도, lat, lon
